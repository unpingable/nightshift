...
fn main() {
    ...
    let admitted_at = Utc
        .with_ymd_and_hms(2026, 8, 30, 16, 2, 1)
        .single()
        .expect("fixed admission time");
    let evaluated_at = Utc
        .with_ymd_and_hms(2026, 8, 30, 16, 3, 0)
        .single()
    ...
    for entry in bundle.entries {
        ...
        let (lineage, _) =
            admit_operational_lineage(&monitor_bytes, &nq_bytes, &input_id, admitted_at, &[])
        ...
        fs::write(directory.join("nq.v1.json"), &nq_bytes).expect("NQ bytes");
        fs::write(
