...
describe("stable casework routes", () => {
  ...
  it("parses every public route and round-trips encoded record ids", () => {
    ...
    expect(parseRoute("/operational-conditions")).toEqual({ kind: "operational-index" });
    expect(parseRoute(operationalConditionPath(digest))).toEqual({
      kind: "operational-condition",
      navigationId: digest,
    });
    expect(parseRoute(operationalQuestionPath(digest, "question/one"))).toEqual({
  ...
  it("fails closed for incomplete and additional path segments", () => {
    ...
    expect(parseRoute(operationalConditionPath(digest) + "/raw/monitor")).toEqual({ kind: "not-found" });
    expect(parseRoute(operationalConditionPath(digest) + "/questions/one/extra")).toEqual({ kind: "not-found" });
    expect(parseRoute(operationalConditionPath(digest) + "/raw/monitor")).toEqual({ kind: "not-found" });
    expect(parseRoute(operationalConditionPath(digest) + "/questions/one/extra")).toEqual({ kind: "not-found" });
  });
});
