...
**Deferred — two parallel unlocks for `EligibleForClosureReview`:**

The originally filed advisory ([`ADVISORY-nq-claim-support.md`](ADVISORY-nq-claim-support.md)) framed the deferred case as "NQ needs to add channel classification on findings." That framing was sharpened by NQ's response (2026-05-27) and the advisory closed via **outcome (2) — recognition**. The corrected model has *two parallel unlocks*, only one of which involves NQ:

1. **Substrate-side claim-support mapping** (small NS-side follow-on). NQ already exposes `cannot_testify + claim_kind + finding_kind`. NS owes a local mapping that tells the closure predicate what kind of substrate support a NQ finding provides. No NQ wire change needed. Lands when a forcing case wants a sharper substrate-blocker refusal than the current `Stale` / `Invalidated` / `ProxyQuiet` set provides.
2. **Consequence-side witness source** (parked architectural question). NQ findings are substrate-state observations by design; consequence-witness (customer-impact, downstream-effect) is application-layer and not in NQ's lane. To reach `EligibleForClosureReview`, a non-NQ consequence-witness source must enter NS's input set. That's a separate architectural concern — new input-source contract, reconciliation interaction, etc. Not on the runtime ladder today.

`EligibleForClosureReview` requires *both* unlocks. Until then, every IncidentShape finding remains `UnassessableMissingConsequenceWitness` (renamed from `UnassessableMissingChannelClassification` post-closeout — see field notes below). The NS side is **complete as a refusal surface**; the missing pieces are inputs, not implementation.

**Post-Slice-4 rename:** `ClosureCandidate::UnassessableMissingChannelClassification` was renamed to `UnassessableMissingConsequenceWitness` after the NQ advisory closeout. The original name encoded the falsified theory ("NQ needs to ship a field"); the new name names what is actually missing (consequence-witness from a non-NQ source). Render label changed from `"unassessable (missing channel classification)"` to `"unassessable (missing consequence-witness)"`. Backward-compat preserved via `#[serde(alias = "unassessable_missing_channel_classification")]` on the new variant.

**Critically — what NS does NOT ask:** NS does not ask NQ to mint closure eligibility. NS owns the predicate; NQ testifies. Pushing NQ to emit `eligible_for_closure` (or any synonym) would turn the witness daemon into a workflow-governance surface, which is the exact failure mode NQ pushed back against in its response.
