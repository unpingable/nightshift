...
- nightshift.provider-capacity-policy/v1 owns thresholds. Thresholds never enter
...
impossible values, contradictions, expiry, and reset rollover produce explicit
UNKNOWN decisions. They never mean unlimited capacity.
...
The only live adapter invokes the installed supported surface:
...
probe has a response-size bound and deadline, drains but does not retain
diagnostics, kills and reaps its own foreground App Server process, and emits
only the normalized record. It does not read provider configuration, session,
browser, or credential files directly; it does not expose login, logout, reset,
consume, or account-mutation methods. Other providers must supply their own
adapter into the same normalized contract.
...
The default policy projects the minimum remaining fraction across all usable
windows:
