...
Unopened refusal details are also owner-closed. Fixed NQ details must match exactly. Parser-derived JSON and timestamp details use a closed grammar limited to the pinned owner messages, exact field names, and the pinned parser error vocabulary. Record and body JSON coordinates are canonical and bounded by the one-MiB Monitor input ceiling; exact payload coordinates are canonical `usize` values because pinned NQ does not bound separately supplied payload bytes. A code/detail substitution or invented diagnostic is refused.
